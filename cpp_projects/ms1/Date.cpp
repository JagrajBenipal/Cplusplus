/* Citation and Sources...
Final Project Milestone 1
Module: Date
Filename: Date.cpp
Version 1.0
Author	Jagraj Singh Benipal
Student ID: 156374217
Revision History
-----------------------------------------------------------
Date      Reason
-----------------------------------------------------------
I have done all the coding by myself and only copied the code
that my professor provided to complete my project milestones.
-----------------------------------------------------------
-----------------------------------------------------------*/
#define _CRT_SECURE_NO_WARNINGS
#include <iomanip>
#include <iostream>
#include <ctime>
using namespace std;
#include "Date.h"
namespace sdds
{
    bool Date::validate()
    {
        errCode(NO_ERROR);
        if (m_year < MIN_YEAR || m_year > m_CUR_YEAR + 1)
        {
            errCode(YEAR_ERROR);
        }
        else if (m_mon < 1 || m_mon > 12)
        {
            errCode(MON_ERROR);
        }
        else if (m_day < 1 || m_day > mdays())
        {
            errCode(DAY_ERROR);
        }
        return !bad();
    }
    int Date::mdays() const
    {
        int days[] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31, -1};
        int mon = m_mon >= 1 && m_mon <= 12 ? m_mon : 13;
        mon--;
        return days[mon] + int((mon == 1) * ((m_year % 4 == 0) && (m_year % 100 != 0)) || (m_year % 400 == 0));
    }
    int Date::systemYear() const
    {
        time_t t = time(NULL);
        tm lt = *localtime(&t);
        return lt.tm_year + 1900;
    }
    void Date::setToToday()
    {
        time_t t = time(NULL);
        tm lt = *localtime(&t);
        m_day = lt.tm_mday;
        m_mon = lt.tm_mon + 1;
        m_year = lt.tm_year + 1900;
        errCode(NO_ERROR);
    }
    int Date::daysSince0001_1_1() const
    { // Rata Die day since 0001/01/01
        int ty = m_year;
        int tm = m_mon;
        if (tm < 3)
        {
            ty--;
            tm += 12;
        }
        return 365 * ty + ty / 4 - ty / 100 + ty / 400 + (153 * tm - 457) / 5 + m_day - 306;
    }
    Date::Date() : m_CUR_YEAR(systemYear())
    {
        setToToday();
    }
    Date::Date(int year, int mon, int day) : m_CUR_YEAR(systemYear())
    {
        m_year = year;
        m_mon = mon;
        m_day = day;
        validate();
    }
    const char *Date::dateStatus() const
    {
        return DATE_ERROR[errCode()];
    }
    int Date::currentYear() const
    {
        return m_CUR_YEAR;
    }
    std::istream &Date::read(std::istream &is)
    {
        char slash;
        errCode(NO_ERROR);
        std::cin >> m_year >> slash >> m_mon >> slash >> m_day;
        if (cin.fail())
        {
            errCode(CIN_FAILED);
            cin.clear();
            cin.ignore(100000000, '\n');
        }
        else if (!validate())
        {
            dateStatus();
            cin.clear();
            cin.ignore(100000000, '\n');
        }
        return is;
    }
    std::ostream &Date::write(std::ostream &os) const
    {
        if (bad())
        {
            cout << DATE_ERROR[m_ErrorCode];
        }
        else
        {
            cout << m_year;
            if (m_mon < 10)
            {
                cout << "/0" << m_mon;
            }
            else
            {
                cout << "/" << m_mon;
            }
            if (m_day < 10)
            {
                cout << "/0" << m_day;
            }
            else
            {
                cout << "/" << m_day;
            }
        }
        return os;
    }
    bool Date::operator==(const Date &ro) const
    {
        return daysSince0001_1_1() - ro.daysSince0001_1_1() == 0;
    }
    bool Date::operator!=(const Date &ro) const
    {
        return daysSince0001_1_1() - ro.daysSince0001_1_1() != 0;
    }
    bool Date::operator>=(const Date &ro) const
    {
        return daysSince0001_1_1() - ro.daysSince0001_1_1() >= 0;
    }
    bool Date::operator<=(const Date &ro) const
    {
        return daysSince0001_1_1() - ro.daysSince0001_1_1() <= 0;
    }
    bool Date::operator<(const Date &ro) const
    {
        return daysSince0001_1_1() < ro.daysSince0001_1_1();
    }
    bool Date::operator>(const Date &ro) const
    {
        return daysSince0001_1_1() > ro.daysSince0001_1_1();
    }
    int Date::operator-(const Date &ro) const
    {
        int diff;
        diff = daysSince0001_1_1() - ro.daysSince0001_1_1();
        return diff;
    }
    Date::operator bool() const
    {
        return !bad();
    }
    void Date::errCode(int readErrorCode)
    {
        m_ErrorCode = readErrorCode;
    }
    int Date::errCode() const
    {
        return m_ErrorCode;
    }
    bool Date::bad() const
    {
        return m_ErrorCode != 0;
    }

    ostream &operator<<(ostream &os, const Date &RO)
    {
        return RO.write(os);
    }
    istream &operator>>(istream &is, Date &RO)
    {
        return RO.read(is);
    }
}